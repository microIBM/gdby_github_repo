...
-- ----------------------------
--  Table structure for `t_statics`
-- ----------------------------
DROP TABLE IF EXISTS `t_statics`;
CREATE TABLE `t_statics` (
...
`date_time` int(11) unsigned NOT NULL DEFAULT '0' COMMENT '哪一天的数据',
...
`first_order_amount` int(11) unsigned NOT NULL DEFAULT '0' COMMENT '首单金额',
`again_order_amount` int(11) unsigned NOT NULL DEFAULT '0' COMMENT '复购金额',
`city_id` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '城市ID',
`customer_type` tinyint(3) unsigned NOT NULL DEFAULT '0' COMMENT '用户类型',
PRIMARY KEY (`id`),
KEY `t_statics_city_id` (`city_id`),
KEY `t_statics_customer_type` (`customer_type`),
KEY `t_statics_index` (`city_id`,`customer_type`)
) ENGINE=InnoDB AUTO_INCREMENT=0 DEFAULT CHARSET=utf8 COMMENT='数据统计总表';
