...
-- ----------------------------
--  Table structure for `t_statics_month`
-- ----------------------------
DROP TABLE IF EXISTS `t_statics_month`;
CREATE TABLE `t_statics_month` (
...
`date_time` int(11) unsigned NOT NULL DEFAULT '0' COMMENT '哪一个月的数据',
...
PRIMARY KEY (`id`),
KEY `t_statics_month_city_id` (`city_id`),
KEY `t_statics_month_customer_type` (`customer_type`),
KEY `t_statics_month_index` (`city_id`,`customer_type`)
) ENGINE=InnoDB AUTO_INCREMENT=0 DEFAULT CHARSET=utf8 COMMENT='数据统计总表-月总计';
