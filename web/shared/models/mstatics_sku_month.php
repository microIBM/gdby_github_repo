...
class MStatics_sku_month extends MY_Model {

    private $_table = 't_statics_sku_month';
    //time是区分数据唯一性的唯一条件
    private $_time = '';
    private $DB_BI;
    ...
    public function update_statics($data = array()) {
        if(!empty($data)) {
            ...
            foreach ($data as $value) {
                if(empty($value['time'])){
                    log_message('error', 'mstatics_sku_month'.print_r($value, TRUE));
                    continue;
                }
     ...
     */
    public function get_month_lists($param = array()){
...
/* End of file statics_sku_month */
/* Location: :./application/models/statics_sku_month.php */
