...
class Order extends MY_Controller {
    ...
    /**
     *
     * @description 检查商品价格是否正常
     */
    private function _check_products_price_and_status_valid($post_products, $user_id) {
        ...
        $db_products = $this->MProduct->get_lists(
            '*',
            array(
                'in' => array(
                    'id' => $post_product_ids
        ...
        //把db里的商品价格进行计算，得到带上折扣的价格
        $db_products = $this->product_price->get_rebate_price($db_products, $user_id, FALSE); //FALSE返回为分

        $db_products_map = array_column($db_products, NULL, 'id');

        $change_flag = FALSE;
        $post_products_map = array();
        foreach($post_products as &$item) {
            $product_key = $item['id'];
            $post_products_map[$product_key] = $item;

            if(!isset($db_products_map[$product_key])) {
                $item['status']      = 4;
                $item['price_in_db'] = 0;
                $change_flag         = TRUE;
                continue;
            }
            $prod_in_db = $db_products_map[$product_key];
            ...
            if($item['price']  > $item['price_in_db']) {
                $item['status'] = 3;
                $change_flag    = TRUE;
            }else if($item['price']  < $item['price_in_db']) {
                $item['status'] = 2;
                $change_flag    = TRUE;
            }
        }
        ...
        foreach($db_products_map as $product_id => $product_info) {
            $product_key = $product_info['id'];
            $db_products_map[$product_key]['quantity'] = $post_products_map[$product_key]['quantity'];
        }

        return array_values($db_products_map);
    }

    //检查订单中的商品是否合法
    private function _check_order_products_valid($user_id) {
        ...
        //检查商品价格是否正常，是否下架
        $valid_products = $this->_check_products_price_and_status_valid($post_products, $user_id);
        ...
        foreach($suborders as $suborder) {
            ...
            foreach($suborder['products'] as $item) {
                $details[] = array(
                    ...
                    'created_time'  => $this->input->server('REQUEST_TIME'),
                    'updated_time'  => $this->input->server('REQUEST_TIME'),
                    'rebate'        => empty($item['rebate']) ? 100 : $item['rebate'],
                    'origin_price'  => empty($item['origin_price']) ? $item['price'] : $item['origin_price'],
                    'sale_id'       => $sale['id'],
                    'sale_role'     => $sale['role_id'],
                );
            }
    ...
    private function _fill_order_extra_info($order_info = array()) {
        ...
        if (isset($deliver_fee_map[$order_info['line_id']])) {
            $deliver_fee_rule = $deliver_fee_map[$order_info['line_id']];
        }
    ...
    */
            ...
            foreach($rules as $rule) {
                ...
                foreach($match_key as $i => $key) {
                    if($i != $len - 1) {
                        ...
                        $tmp_minus_amount = ceil($prod_price / $all_amount * $minus_amount);
                        $products[$key]['minus_amount'] += $tmp_minus_amount;
                        $now_minus_amount -= $tmp_minus_amount;
    ...
    public function add() {
        ...
        $valid_products = $this->_check_order_products_valid($user['id']);
        ...
        if ($user['invite_id'] == C('customer.public_sea_code')) {
            ...
            if($invite_bd){
                ...
                if (!empty($order_id)) {
                    ...
                    $this->MOrder_detail->update_info(['sale_id' => $user['invite_bd'], 'sale_role' => C('user.saleuser.BD.type')], ['order_id' => $order_id]);
                }
            }
        }
        ...
        $this->MCustomer->update_info(
            array(
                'latest_ordered_time' => $this->input->server('REQUEST_TIME')
            ),
            array(
                'id' => $user_id
            )
    ...
    private function _format_order_list($order_list = array()) {
        ...
        foreach($suborders as $suborder) {
            ...
            if(empty($order_id_deposits[$order_id])) {
                $order_id_deposits[$order_id] = $suborder['deposit'];
            } else {
                $order_id_deposits[$order_id] += $suborder['deposit'];
            }
        }
        ...
        foreach($order_list as &$item) {
            ...
            $item['deposit'] = empty($order_id_deposits[$order_id]) ? 0 : $order_id_deposits[$order_id];
        }
        unset($item);
    ...
    public function info() {
        ...
        foreach($suborders as &$suborder) {
            ...
            $order['deposit'] += $suborder['deposit'];
            $suborder['deposit'] = sprintf('%.2f', $suborder['deposit']);
    ...
    public function set_status_closed() {
        ...
        $order_id = intval($_POST['order_id']);
        $user     = (array) $_POST['cur'];
        $is_customer = ! isset($user['dept_id']);
        //检测操作用户的合法性
        ...
        foreach($statuses as $status) {
            //如果为客户只有未审核状态才能取消订单
            if ($is_customer && $status != C('order.status.wait_confirm.code')) {
                $pass_flag = FALSE;
                break;
            }
