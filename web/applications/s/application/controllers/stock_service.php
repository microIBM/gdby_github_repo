...
class Stock_service extends MY_Controller {
    ...
    public function set_virtual_stock() {
        if(!$_POST['sku_number']|| !$_POST['warehouse_id'] || !isset($_POST['virtual_stock'])) {
            $this->_return_json(['status' => 0, 'msg' => 'sku_number和warehouse_id不能为空,virtual_stock也不能为空']);
        }

        $sku_number = $_POST['sku_number'];
        $warehouse_id = $_POST['warehouse_id'];
        $value = $_POST['virtual_stock'];
        $update_res = $this->MStock->update_info(
            array(
                'virtual_stock' => $value
            ),
    ...
    /**
     * @description 设置某个sku的库存，如果库存表里没有这条记录，那么插入，并返回，如果有，那么返回原来的记录
     */
    public function set_sku_initial_stock() {
        if(empty($_POST['sku_number']) || empty($_POST['warehouse_ids']) || !is_array($_POST['warehouse_ids'])) {
            $this->_return_json(['status' => -1, 'msg' => '仓库id和sku_number不能为空，warehouse_ids必须为数组']);
        }
        ...
        foreach($warehouse_ids as $wh_id) {
            $record_id = $this->MStock->check_and_insert(
                array(
                    'sku_number' => $sku_number,
                    'warehouse_id' => $wh_id,
            ...
            $record_id_arr[] = $record_id;

        }


        $this->_return_json(['status' => 0, 'insert_res' => $record_id, 'msg' => 'success']);
    }
