...
class Coupon extends MY_Controller {
    ...
    private function _deal_coupon_data(&$data) {
        $category_ids_arr = array_column($data, 'category_ids');
        $new_category_ids = '';
        ...
        if(!empty($new_category_ids)) {
            $new_category_ids = trim($new_category_ids, ',');
            $new_category_ids = explode(',', $new_category_ids);
            ...
            if(!empty($category_lists)) {
                $category_lists = array_column($category_lists, 'name', 'id');
            }
        }

        $coupon_trigger = array_column(C('coupon_things_type.trigger'), 'name', 'id');
        ...
        foreach($data as &$dv) {
            ...
            $dv['coupon_trigger_cn'] = $coupon_trigger[$dv['coupon_type']];
            $dv['location_cn'] = $new_locations[$dv['location_id']]['name'];
            if(!empty($dv['category_ids'])) {
                $dv['category_cn'] = '';
                $tmp_category_ids = explode(',', $dv['category_ids']);
                foreach($tmp_category_ids as $tmp_id) {
                    $dv['category_cn'] .= isset($category_lists[$tmp_id]) ? $category_lists[$tmp_id].',' : '';
                }
                $dv['category_cn'] = trim($dv['category_cn'], ',');
            } else {
                $dv['category_cn'] = '全品通用';
            }
        }
