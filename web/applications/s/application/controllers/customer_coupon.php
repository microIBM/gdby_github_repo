...
class Customer_coupon extends MY_Controller {
    ...
    public function lists() {
        ...
        if(isset($_POST['status']) && $_POST['status'] === '0') {
            ...
            $where = "valid_time <= {$current} AND invalid_time >= {$current} and status={$status}";
        } else {
            //查询所有
            $where = "status != 0";
        }
        ...
        if(isset($_POST['where']['customer_id'])) {
            $customer_total_where = array('customer_id' => $_POST['where']['customer_id'], 'status != ' => 0);
        }
        ...
        if($coupons) {
            ...
            foreach($data as &$v) {
                ...
                $v['detail'] = array(
                    ...
                    'rule_type'    => $detail['rule_info']['rule_type'],
