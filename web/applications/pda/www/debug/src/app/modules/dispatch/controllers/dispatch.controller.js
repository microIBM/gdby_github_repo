...
angular
.module('pda')
.controller('DispatchCtrl', ['$scope', 'req', '$location', '$cookieStore','daChuLocal', 'HopAuth', function($scope, req, $location, $cookieStore, daChuLocal, HopAuth) {
  // 数据等待
  $scope.wait = false;
  //让扫码框自动对焦
  ...
  var callBack = function(data) {
    ...
    if(parseInt(data.status) === 0) {
      if(common.length(data.data) > 0){
        daChuLocal.set('dispatch_info', data.data);
        req.redirect('dispatch_confirm/'+data.data.prefix+data.data.dist_number);
      }else{
       $scope.error = {cls:'alert alert-danger text-center', msg:'无效的配送单号'};
     }
   } else {
