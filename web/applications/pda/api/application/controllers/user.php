...
class user extends MY_Controller {
    ...
    public function update_password() {
        ...
        $old_password_p = $this->input->post('password');
        $new_password_p = $this->input->post('newPassword');
        
        
        $result = $this->MUser->get_user_info(array("mobile" => $mobile));
        //$this->_return_json($result);exit;
        
        if(!empty($result)) {
        	$cur['password'] = $result['password'];
        	$cur['salt'] = $result['salt'];
        	$cur['id'] = $result['id'];
        	$old_password_p = $this->create_password($old_password_p, $cur['salt']);
        	if($old_password_p == $cur['password'] ) {
        		// 修改用户密码
        		$new_password = $this->create_password($new_password_p, $cur['salt']);
        		$result = $this->MUser->update_by('id', $cur['id'], array('password' => $new_password));
        		$this->_return($result);
        	} else {
        		$this->_return_json(
        				array(
