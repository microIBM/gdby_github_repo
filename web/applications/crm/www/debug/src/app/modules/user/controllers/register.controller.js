...
.module('dachuwang')
// 用户注册
.controller('UserRegisterCtrl', ['$scope', 'rpc', '$cacheFactory', '$timeout', '$location', 'daChuDialog', '$cookieStore', 'daChuLocal', 'daChuTimer', function($scope, req, $cacheFactory, $timeout, $location, daChuDialog, $cookieStore, daChuLocal, daChuTimer) {
  var pattern = /\d+/;
  ...
  if(daChuLocal.get('regUserType') && daChuTimer.compare(daChuLocal.get('regUserTypeTime'))) {
    ...
    rpc.getdata('user/user_type', 'POST', function(data) {
  ...
  */
  // 设置缓存
  rpc.getdata('user/register_options', 'GET',function(data) {
  ...
  $scope.sms = function() {
    if($scope.reg.mobile == '') {
      ...
      rpc.getdata('user/smscode', 'POST', function(data) {
        if(data.status === 0) {
  ...
  $scope.register = function() {
    ...
    rpc.getdata('user/register', 'POST', function(data) {
      var bodyText = '恭喜您注册成功';
      if(data.status === 0) {
        if($scope.reg.type == 20) {
          $cookieStore.put('type', 20);
        } else {
          bodyText = '恭喜您注册成功，请等待大厨网审核。有任何疑问请联系010-58298105。';
        }
