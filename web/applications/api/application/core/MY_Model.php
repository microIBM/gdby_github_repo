...
class MY_Model extends CI_Model {
    ...
    protected function delete($query) {
        if($this->db->where($query)->update($this->_table,array('status' => C('status.order.del')))) {
                $info = array(
                    'status'    => TRUE,
                    'message'   => '删除成功'
                );
...
class ODOO_Model extends CI_Model {
    ...
    /*
    @function 封装odoo底层函数
    @param    $model_name 对象名称
    @param    $action    动作名称
    @param    $args    第一个参数，通常是一个list形式的数组
    @param    $kw_args    第二个参数，通常时一个key-value形式的数组

    */
    public function exe_rpc_call($model_name = '',$action = '',$args = array(),$kw_args = array()) {

        if(!array_key_exists('context', $kw_args)){
        	$kw_args['context']=array();
        }
        if(!array_key_exists('lang', $kw_args['context'])){
        	$kw_args['context']['lang']='zh_CN';
        }
        if(!array_key_exists('uid', $kw_args['context'])){
        	$kw_args['context']['uid']=$this->uid;
        }

        $resp = $this->models->execute_kw($this->odoo_db,$this->uid,$this->odoo_password,$model_name,$action,
             $args,$kw_args);
        return $resp;
    }
