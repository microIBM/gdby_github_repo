'use strict';
angular.module('dachuwang').directive('dachuPagination',[function(){
    return {
        restrict: 'EA',
        template: '<div class="page-list">' +
            '<ul class="pagination" ng-show="conf.totalItems > 0">' +
            '<li ng-class="{disabled: conf.currentPage == 1}" ng-click="prevPage()"><span class="prev">&laquo;</span></li>' +
            '<li ng-repeat="item in pageList track by $index" ng-class="{active: item == conf.currentPage, separate: item == \'...\'}" ' +
            'ng-click="changeCurrentPage(item)">' +
            '<span>{{ item }}</span>' +
            '</li>' +
            '<li ng-class="{disabled: conf.currentPage == conf.numberOfPages}" ng-click="nextPage()"><span class="next">&raquo;</span></li>' +
            '</ul>' +
            '<div class="page-total m-r-5" ng-show="conf.totalItems > 0">' +
            '第<input type="text" ng-model="jumpPageNum" class="m-lr-5"  ng-keyup="jumpToPage($event)"/>页 ' +
            '每页<select class="form-control" ng-model="conf.itemsPerPage" ng-options="option for option in conf.perPageOptions " ng-change="changeItemsPerPage()"></select>' +
        ...
        link: function(scope, element, attrs){
            ...
            function getPagination(){
                ...
                for(var i = 0; i < perPageOptionsLength; i++){
                    if(parseInt(scope.conf.perPageOptions[i]) == parseInt(scope.conf.itemsPerPage)) {
