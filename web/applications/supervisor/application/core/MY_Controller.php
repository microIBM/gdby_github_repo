...
class MY_Controller extends CI_Controller {
    ...
    /*
     * xmlrpc supervisor 接口请求统一方法
     * @param $server array
     * @param $api string
     * $parameters array 传递的数组；
     * @author fengzongbao@dachuwang.com
     */
    public function _xmlrpcrequest($server_name, $api, $parameters)
    {
        $server = C('supervisor.servers.' . $server_name);
        if(empty($server)) {
            die('supervisor server config is wrong');
        }
        $this->xmlrpc->server($server['url'], $server['port']);
        $this->xmlrpc->method('supervisor.' . $api);

        if(isset($server['username']) && isset($server['password'])){
            $this->xmlrpc->setCredentials($server['username'], $server['password']);
        }

        $this->xmlrpc->request($parameters);
