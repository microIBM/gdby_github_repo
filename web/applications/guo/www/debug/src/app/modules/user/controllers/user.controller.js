...
// 用户编辑
.controller('UserEditCtrl', ['$scope', 'req', '$cookieStore', 'daChuDialog', function($scope, req, $cookieStore, daChuDialog) {
  ...
  req.getdata('user/getinfo', 'POST', function(data) {
  ...
  $scope.get_market = function(parent) {
    if(parent = 'province'){
      if($scope.addr.province.id <= 4) {
        market_query_flag = 1;
        req.getdata('market/get_market', 'POST',function(data) {
          $scope.markets = data;
  ...
  $scope.save = function() {
    ...
    req.getdata('/user/update_myself', 'POST', function(data) {
      if(data.status) {
